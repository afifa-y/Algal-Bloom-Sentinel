...
if "selected_site_no" not in st.session_state:
    st.session_state["selected_site_no"] = "05114000"  # Souris River near Sherwood, ND


def _use_search_selection():
    label = st.session_state.get("search_selectbox")
    options = st.session_state.get("search_options", {})
    if label in options:
        st.session_state["selected_site_no"] = options[label]


def _use_manual_entry():
    val = st.session_state.get("manual_site_input", "").strip()
    if val:
        st.session_state["selected_site_no"] = val
...
with search_tab:
    ...
    if st.button("Search"):
        with st.spinner(f"Searching USGS sites in {US_STATES[state_code]}..."):
            try:
                ...
                # auto-select the first match right away
                first_label = list(options.keys())[0]
                st.session_state["search_selectbox"] = first_label
                st.session_state["selected_site_no"] = options[first_label]
    ...
    if "search_results" in st.session_state:
        ...
        st.selectbox(
            "Matching sites", options=list(options.keys()),
            key="search_selectbox", on_change=_use_search_selection,
        )
...
with manual_tab:
    ...
    st.text_input(
        "USGS site number", value="05114000",
        key="manual_site_input", on_change=_use_manual_entry,
    )

site_no = st.session_state["selected_site_no"]
st.caption(f"Selected site number: **{site_no}**")
